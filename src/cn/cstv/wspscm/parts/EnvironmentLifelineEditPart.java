...
import cn.cstv.wspscm.figure.EnvironmentLifelineFigure;
 ...
 * 
 */
public class EnvironmentLifelineEditPart extends EditPartWithListener implements
		NodeEditPart {

	private EnvironmentLifelineFigure figure;
	...
	protected IFigure createFigure() {					//确定外观
		...
		EnvironmentLifeline lifeline = (EnvironmentLifeline) getModel();		//取得模型
		figure = new EnvironmentLifelineFigure(lifeline);
		return figure;
	}
	...
	protected void refreshVisuals() {
		...
		Rectangle rectangle = new Rectangle(((EnvironmentLifeline) getModel())
	...
	protected void refreshSizeVisuals() {
		Rectangle rectangle = new Rectangle(((EnvironmentLifeline) getModel())
				.getLocation(), ((EnvironmentLifeline) getModel()).getSize());
		((GraphicalEditPart) getParent()).setLayoutConstraint(this,
				getFigure(), rectangle);
	}

	public void propertyChange(PropertyChangeEvent event) {
		if (event.getPropertyName().equals(EnvironmentLifeline.P_LOCATION)) {
			refreshSizeVisuals();
		} else if (event.getPropertyName().equals(EnvironmentLifeline.P_SIZE)) {
			refreshSizeVisuals();
		} else if (event.getPropertyName().equals(EnvironmentLifeline.P_NAME)) {
			((Label) getFigure()).setText((String) event.getNewValue());
			refreshVisuals();
		} else if (event.getPropertyName().equals(EnvironmentLifeline.P_SOURCE)) {
			refreshSourceConnections();
		} else if (event.getPropertyName().equals(EnvironmentLifeline.P_TARGET)) {
			refreshTargetConnections();
		}else if (event.getPropertyName().equals(EnvironmentLifeline.P_OPERATOR)) {
			refreshOperators();
		}
	}
...
//		Rectangle rect = figure.getTextBounds();
	...
	public List getModelSourceConnections() {
		return ((EnvironmentLifeline) getModel()).getSourceConnections();
	}

	@SuppressWarnings("unchecked")
	public List getModelTargetConnections() {
		return ((EnvironmentLifeline) getModel()).getTargetConnections();
	}
}
