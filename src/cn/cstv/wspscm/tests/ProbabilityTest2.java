...
public class ProbabilityTest2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("输入执行结果，以0和1的序列形式：");
		...
		int mm = SequentialProbabilityRatioTest(p0, p1, alpha, beta, sequence);
		int no = sequence.size();
		if (Math.abs(mm) < no) {
			if (mm >= 0) {
				System.out.println("m=" + mm + "时接受H0");
			}
			if (mm < 0) {
				System.out.println("m=" + (-mm) + "时接受H1");
			}
		} else if (Math.abs(mm) >= no) {
			System.out.println("均不接受H0和H1");
		}
